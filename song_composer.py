...
class InstrumentConfig:
    ...
    # Section behavior
    play_in_sections: list[SectionType] = None  # Which sections this instrument plays in
...
class EvolvedSection:
    ...
    phrases: dict[str, Phrase] = field(default_factory=dict)  # instrument_name -> phrase
    chords: dict[str, ChordProgression] = field(default_factory=dict)  # instrument_name -> chords
...
class SongComposer:
    ...
    def _evolve_rhythm(
        ...
        max_gens = self.max_generations if self.fitness_threshold else self.rhythm_generations
        
        for gen in range(max_gens):
            population = ga.evolve(
                population=population,
                fitness_fn=rhythm_fitness,
                mutate_fn=lambda r: mutate_rhythm(r, self.mutation_rate, effective_max_sub),
                crossover_fn=crossover_rhythm,
            )
            
            # Check threshold
            if self.fitness_threshold and population[0].fitness >= self.fitness_threshold:
                if verbose:
                    print(f"       ✓ Reached threshold {self.fitness_threshold:.2f} at gen {gen+1} (fitness: {population[0].fitness:.3f})")
                break
    ...
    def _evolve_melody(
        ...
        population = [
            Individual(
                rhythm_to_phrase(rhythm, scale=instrument.scale, octave_range=instrument.octave_range)
            )
            for _ in range(self.population_size)
        ]
        ...
        # Use threshold-based or fixed generations
        max_gens = self.max_generations if self.fitness_threshold else self.melody_generations
    ...
    def _evolve_chords(
        ...
        # Use threshold-based or fixed generations
        max_gens = self.max_generations if self.fitness_threshold else self.chord_generations
    ...
    def evolve_section(self, section_type: SectionType, verbose: bool = True) -> EvolvedSection:
        """Evolve all instruments for a single section."""
        section_config = self._get_section_config(section_type)
        
        if verbose:
            print(f"\n{'='*60}")
            print(f"🎵 Evolving {section_type.value.upper()} section ({section_config.bars} bars)")
        ...
        for instrument in self.instruments:
            ...
            if instrument.is_drum:
                ...
                rhythm, fitness = self._evolve_rhythm(instrument, section_config, verbose)
                ...
                # Chord layer
                chords, fitness = self._evolve_chords(instrument, section_config, verbose)
                evolved.chords[instrument.name] = chords
                if verbose:
                    chord_summary = " → ".join(f"deg{c.root_degree}" for c in chords.chords)
                    print(f"     Chords: {chord_summary} (fitness: {fitness:.3f})")
            
            else:
                # Melodic instrument
                rhythm, r_fitness = self._evolve_rhythm(instrument, section_config, verbose)
                evolved.rhythms[instrument.name] = rhythm
                
                phrase, m_fitness = self._evolve_melody(instrument, section_config, rhythm, verbose)
    ...
    def _section_to_strudel(self, section: EvolvedSection, section_idx: int) -> list[str]:
        ...
        for instrument in self.instruments:
            ...
            if instrument.is_drum:
                ...
                if rhythm:
                    ...
                    for beat_char in rhythm:
                        ...
                        if subdivisions == 0:
                            ...
                            beat_groups.append("[" + " ".join([instrument.drum_sound] * subdivisions) + "]")
                ...
                if rhythm and phrase:
                    pattern = phrase.to_strudel_with_rhythm(rhythm, instrument.use_scale_degrees)
    ...
    def get_strudel_code(self, bpm: int = 120) -> str:
        ...
        lines.append(f"// Structure: {' → '.join(s.value for s in self.song_structure)}")
        ...
        for instrument in self.instruments:
            ...
            for section_type, evolved in self.evolved_sections.items():
                ...
                if instrument.is_drum:
                    ...
                    if rhythm and phrase:
                        patterns_by_section[section_type] = phrase.to_strudel_with_rhythm(
                            rhythm, instrument.use_scale_degrees
                        )
                    else:
                        patterns_by_section[section_type] = "~"
            
            # Build the full song pattern by concatenating section patterns
            # Using <...> for sequence in Strudel
            section_patterns = []
            for start, end, section_type in section_instances:
                bars = end - start
                pattern = patterns_by_section.get(section_type, "~")
                # Wrap pattern to repeat for the section length
                section_patterns.append(f"[{pattern}]")
            
            full_pattern = " ".join(section_patterns)
        ...
        for section_type, evolved in self.evolved_sections.items():
            ...
            for instrument in self.instruments:
                ...
                if instrument.is_drum:
                    ...
                    chords = evolved.chords.get(instrument.name)
                    if chords:
                        chord_str = " → ".join(f"deg{c.root_degree}" for c in chords.chords)
